...
@section('content')
    ...
    <div class="mx-auto max-w-2xl px-6 py-10 md:py-14">
        ...
        <form
            ...
            class="medrek-hover-lift rounded-2xl border border-[var(--gov-border)] bg-gradient-to-b from-white to-slate-50/80 p-6 shadow-[0_8px_30px_rgba(15,23,42,0.06)] md:p-8"
            x-data="registrationWizard({{ (int) $initialWizardStep }})"
            @input="formTick++"
            @change="formTick++"
            @focusout="formTick++"
            @submit="if (!validateStep()) { $event.preventDefault(); }"
        >
            @csrf

            <div
                class="mb-8 h-2 overflow-hidden rounded-full bg-slate-100"
                role="progressbar"
                aria-label="تقدّم التسجيل"
                aria-valuemin="1"
                x-bind:aria-valuemax="totalSteps"
                x-bind:aria-valuenow="step"
                x-bind:aria-valuetext="`الخطوة ${step} من ${totalSteps}`"
            >
                <div
                    class="h-full rounded-full bg-[var(--gov-accent)] transition-all duration-500 ease-out"
            ...
            >
                <fieldset>
                    ...
                    <label
                        ...
                        class="mt-6 flex cursor-pointer items-start gap-3 rounded-xl border border-[var(--gov-border)] bg-white p-4 transition hover:border-[var(--gov-accent)]/30 hover:bg-[var(--gov-accent-soft)]/25 sm:gap-3.5 sm:p-4"
                    >
                        <input
                            id="program-details-ack"
                            type="checkbox"
                            x-model="viewedProgram"
            ...
            return {
                ...
                validateStep() {
                    ...
                    if (this.step === 3) {
                        ...
                        if (!v) { this.clientErrors.education_stage = 'يرجى اختيار المرحلة الدراسية.'; return false; }
                        return true;
                    }
                    if (this.step === 4) {
                        let ok = true;
                        if (!document.getElementById('gender')?.value) { this.clientErrors.gender = 'يرجى اختيار الجنس.'; ok = false; }
                        if (!document.getElementById('region')?.value) { this.clientErrors.region = 'يرجى اختيار المنطقة.'; ok = false; }
                        return ok;
                    }
                    if (this.step === 5) {
                    ...
                    if (this.step === 6) {
                        const ref = document.getElementById('referral_source')?.value;
                        if (!ref) { this.clientErrors.referral_source = 'يرجى اختيار مصدر المعرفة.'; return false; }
                        if (ref === 'أخرى') {
                            const o = document.getElementById('referral_source_other')?.value?.trim();
                            if (!o) { this.clientErrors.referral_source_other = 'يرجى التوضيح.'; return false; }
                        }
                        return true;
                    }
                    return true;
